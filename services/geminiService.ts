...
const getClassName = (id: string) => {
  ...
  return c ? `${c.grade} - ${c.section} (${c.wing})` : id;
};
...
export const generateDailyReport = async (logs: ClassAttendanceLog[]): Promise<string> => {
  try {
    ...
    if (notes.length === 0 && totalAbsentees === 0 && totalLate === 0) {
      return "All students across wings are present today. No critical disciplinary issues reported. A smooth day.";
    }
    ...
    const response = await ai.models.generateContent({
      ...
      contents: `
        You are an assistant to the Head of School (HOS) at GSIS. 
        Analyze the following daily attendance and disciplinary data covering MYP, MS, HS, and HSS sections.
        ...
        2. Highlight patterns in attendance (e.g., "High lateness in HS Wing" or "Attendance issues in MYP").
